...
class CombinatorialProofs:
    ...
    @staticmethod
    def brute_force_proof(goal: LogicalExpression, premises: List[LogicalExpression], max_depth: int = 5) -> Optional[Proof]:
        """
        ...
        proof_steps = [ProofStep(step_number=i + 1, statement=premise, justification="Given") for i, premise in enumerate(premises)]

        for depth in range(1, max_depth + 1):
            for perm in permutations(list(known_statements.keys()), 2):  # Try all 2-combinations of premises
                ...
                for new_statement in derived_statements:
                    if new_statement and new_statement not in known_statements.values():
                        ...
                        proof_steps.append(
                            ProofStep(step_number=step_number, statement=new_statement, justification="Derived", dependencies=[perm[0], perm[1]])
                        )

                        if new_statement == goal:
                            return Proof(steps=proof_steps)  # Return proof if goal is reached
